<template>
  <div class="px-6 py-phi-3 max-w-6xl mx-auto">
    <div class="mb-phi-3">
      ...
      <h1 class="text-phi-h2 font-bold text-olive mb-4">Best Sellers</h1>
      ...
      <div class="flex flex-col sm:flex-row gap-3">
        ...
        <div class="flex items-center bg-white border border-olive/15 rounded-full px-4 py-2.5 gap-2 shrink-0">
          ...
          >
            <option value="rank">Top Selling</option>
    ...
    <div v-else-if="products.length === 0" class="text-center py-phi-4">
      <Icon name="mdi:trophy-outline" class="text-5xl text-olive/20 mb-4" />
      <p class="text-olive font-semibold mb-1">No best sellers yet</p>
      <p class="text-sm text-taupe">Once orders start coming in, top products will show up here.</p>
    </div>
    ...
    <div v-else v-fade-in class="grid grid-cols-2 md:grid-cols-4 gap-phi-2">
      ...
      >
        <div class="relative aspect-square bg-champagne overflow-hidden">
          ...
          <span
            class="absolute top-2 left-2 text-xs font-bold px-2 py-1 rounded-full bg-olive text-beige"
          >
            Best Seller
          </span>
...
useSeoMeta({
  title: 'Best Sellers',
  description: 'Shop our most popular products at Saadawy Store — ranked by real customer orders.',
  ogTitle: 'Best Sellers | Saadawy Store',
  ogDescription: 'Shop our most popular products at Saadawy Store — ranked by real customer orders.'
})
