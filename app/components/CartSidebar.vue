  ...
  <Teleport to="body">
    <transition name="fade">
      <div v-if="cartUI.isOpen" class="fixed inset-0 z-[100]">
        ...
        <div class="absolute top-0 right-0 h-full w-full sm:w-96 bg-white flex flex-col shadow-xl">
          <div class="flex items-center justify-between p-4 border-b border-olive/10">
          ...
          <div v-if="cart.items.length === 0" class="flex-1 flex flex-col items-center justify-center p-8 text-center">
            ...
            <p class="text-sm text-taupe mb-6">Looks like you haven't added anything yet.</p>
            <button
          ...
          <template v-else>
            <div class="flex-1 overflow-y-auto p-4 space-y-4">
              ...
              >
                <div class="w-16 h-16 rounded-lg bg-champagne shrink-0 flex items-center justify-center overflow-hidden">
                  <img
                    v-if="item.image"
                    :src="item.image"
                    :alt="item.name"
                  ...
                  />
                  <Icon v-else name="mdi:image-outline" class="text-2xl text-olive/30" />
                </div>

                <div class="flex-1">
                  <p class="text-sm font-medium text-olive mb-1">{{ item.name }}</p>
            ...
            <div class="p-4 border-t border-olive/10">
              <div class="flex items-center justify-between mb-4">
                ...
                <span class="font-bold text-olive text-lg">EGP {{ cart.subtotal }}</span>
              </div>
              <button class="w-full bg-olive text-beige py-3 rounded-full font-semibold hover:bg-gold hover:text-olive transition">
                Checkout
              </button>
            </div>
          </template>
